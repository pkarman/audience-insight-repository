...
SKIP: {
    skip( "skipping mailchimp", 80 ) unless MailchimpUtils::env_ok();
 ...
 #            added the unsub to MC, and we actively nuke unsubs from MC)
    ...
    $src_emails[5]
        ->sem_upd_dtim( $src_emails[5]->sem_upd_dtim->epoch() - 200 );
    ...
    #    expect: soe_status(U), MC-unsubscribed
    $src_emails[7]
        ->sem_upd_dtim( $src_emails[7]->sem_upd_dtim->epoch() + 200 );
    ...
    $src_emails[8]
        ->sem_upd_dtim( $src_emails[8]->sem_upd_dtim->epoch() + 200 );
    ...
    is( $src_org_emails[10]->soe_status, 'U',
        '10 AIR-wins-tie - soe_status' );
    ...
    is( scalar @{ $sources[11]->src_orgs },
        0, '11 src_org DNE - so_count after' );
    is( scalar @{ $src_emails[11]->src_org_emails },
